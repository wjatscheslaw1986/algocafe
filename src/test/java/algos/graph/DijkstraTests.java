...
public class DijkstraTests {
    ...
    public void aStarCrossroadsWeightedAdjacencyMatrixGraphTest() throws GraphInstantiationException {
        ...
        GraphUtil.DijkstraResult<WeightedRib> forth;

        forth = GraphUtil.<CrossroadsNode, WeightedRib, CrossroadsWeightedAdjacencyMatrixGraph<CrossroadsNode, WeightedRib>>dijkstra(graph.nodeAt(graph.indexOf(Crossroad.BOLSHOY_PROSPECT_18_19_LINES)), graph);
        Assertions.assertNotNull(forth);

        //System.out.println("FORTH: " + PathNodeUtil.nodeToPath(forth));
        System.out.println("The Dijkstra algorithm took " + stepsCounter.get() + " steps to complete on " + graph.getNodeCount() + " nodes.");
    }
}
