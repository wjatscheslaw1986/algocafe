...
public class PathNodeUtil {
	...
	public static <T> PathNode<T> bfs(T initial, Predicate<T> goalTest, Function<T, List<T>> successors, AtomicInteger stepsCounter) {
	...
	public static <T> PathNode<T> astar(T initial, Predicate<T> goalTest, Function<T, List<T>> successors, ToDoubleFunction<T> heuristic, AtomicInteger stepsCounter) {
		...
		while (!frontier.isEmpty()) {
			...
			for (T child : successors.apply(currentState)) {
				double newCost = currentNode.cost + 1.0d; // let's assume equal cost = 1.0d for all tiles
